...
// Image definition to use as the bus
var busImage = new google.maps.MarkerImage('images/bus_popup.png',
 ...
 */
function moveBusMarker(latlng) {
	console.log(JSON.stringify(latlng));
	console.log(latlng.lat());
	console.log(latlng.lng());

	busMarker.animateTo(latlng, {easing: 'linear', duration: 1500});
}
...
function processRoutes(json) {
    ...
    console.log(JSON.stringify(routes[i]));
		...
		bindInfoWindow(busMarker,Demo.map, html, "route"+ i );
...
function showRoute(json) {
	...
	for (x = 0; x < json.routelist.length; x++) {
		...
		if (x == 0) {
			// First stop, let's create the marker here
			busMarker = new google.maps.Marker({
				map: Demo.map,
				position: latlng,
				shadow: shadow,
...
function dateToInt() {
	...
	var dayVal;
  // 0 is sunday!
  dayVal = parseInt($("#day").val()) * 1440;
  console.log("dayVal is " + dayVal);
