...
public class ShortestPathGeo {
	...
	public ShortestPathGeo(int startTime, int _timeInterval, double lat1,
			double lon1, double lat2, double lon2, int walkLimit,
			Criteria criterion) {
	...
	public String toString() {
		...
		for (Direction dir : arrivalList) {
			...
			if (arrivo != null) {
				...
				outPath = "(WALK: t:" + dir.getWalkTime() + "|arrtime:"
						+ dir.getArrivalTime() + "|distwalk:"
						+ dir.getDistance() + "|numchanges:"
						+ dir.getNumChanges() + ")";
				dirstr = outPath;

				LASTsTAZ = "(" + arrivo.getUnderlyingNode().getId() + ":ID"
						+ arrivo.getId() + ":STAZID"
						+ arrivo.getStation().getId() + ":RUNID"
						+ arrivo.getRun().getId() + ":TIME" + arrivo.getTime()
						+ ")-->";
				...
				while (arr.prevSP != null) {
					outPath = "("
							...
							+ arr.getId()
							+ ":IDNODE"
							+ arr.getUnderlyingNode().getId()
							+ ":STAZID"
							+ arr.getStation().getId()
							+ ":RUNID"
							+ (arr.getRun() != null
									? arr.getRun().getId()
									: "NN") + ":TIME" + arr.getTime() + ")-->"
				...
				outPath = "(" + arr.getType() + ":" + arr.getId() + ":STAZID"
						+ arr.getStation().getId() + ":RUNID" + "NN" + ":TIME"
						+ arr.getTime() + ")-->" + outPath;

			}
			strPath = strPath + dir + "\n\n" + dirstr + ":\tARRIVAL:"
					+ LASTsTAZ + "dep:" + dir.getDepartureTime() + "walk:"
					+ dir.getWalkDistance() + "\t\t" + outPath;
		}
	...
	private void debug(String s) {
		try {
			FileWriter logFile = new FileWriter(
					"/tmp/createTransientStops.log", true);
	...
	private void createTransientStops() {
		Date start = new Date();
		Collection<Station> startStations = Stations.getStations()
				.getNearestStations(lat1, lon1, walkLimit);

		int stopsNumber = 0;
		this.debug("startStations:" + startStations.size() + "\n");

		for (Station s : startStations) {
			double distance = GeoUtil.getDistance2(lat1, lon1, s.getLatitude(),
					s.getLongitude());
			...
			while (startStop != null) {
				...
				this.debug("stop:" + startStop.getId() + "\n");
		...
		this.debug("stopsNumber: " + stopsNumber + ", in (ms): " + diff + "\n");

	}

	private void linkArrivalPoint() {
		/*
		 * collega l'arrival point agli stop delle stazioni adiacenti tramite
		 * archi walk
		 * 
		 * collegando tutti gli stop delle stazioni adiacenti (entro WALKLIMIT),
		 * 
		 * Infine per ogni stazione, per ogni percorso trovato, crea una lista
		 * di risultati. I risultati si inseriscono in ordine di orario di
		 * arrivo, quindi secondo l'ordine dei nodi nelle rispettive stazioni.
		 * 
		 * Quindi si procede a fare un merge-sort di tutte le liste di tutte le
		 * stazioni di arrivo, tenendo ordinati i risultati per arrival-time e
		 * scartando on-the-fly i risultati che non sono ottimi di pareto. ==>
		 * arrivalList
		 * 
		 * Gli ottimi si valutano in base al secondo criterio di ottimizzazione
		 * scelto. Dati 2 Stop di arrivo successivi s1 e s2, con t(s1) < t(s2),
		 * allora s2 e' un ottimo di pareto se SECONDOCRITERIO(s2) <
		 * SECONDOCRITERIO(s1)
		 * 
		 * NB: nell'attuale implementazione il secondo criterio di
		 * ottimizzazzione e' la composizione di tutti i criteri ammissibili,
		 * mettendo prima il secondo criterio scelto e, a seguire tutti gli
		 * altri criteri
		 */

		LinkedList<Direction> arrivals = new LinkedList<Direction>();
		Collection<Station> arrivalStations = Stations.getStations()
				.getNearestStations(lat2, lon2, walkLimit);

		// System.out.print("\nLnked arrival stations: " +
		// arrivalStations.size());
		for (Station s : arrivalStations) {
			...
			while (arrivalStop != null) {
				...
				// while(arrivalStop != null && arrivalStop.nextInStation !=
				// null &&
				// arrivalStop.equals(arrivalStop.nextInStation.prevSP)){
		...
		for (Direction d : arrivals) {
			// System.out.print("\n\n---------- Comparing: \nd:\n" + d +
			// "\nprev:\n" + prev + "\nRIS: " +comp.secondCriterionCompare(d,
			// prev) + "\n-------\n\n");
	...
	private int compareMinTravelTime(Stop s, Stop next) {
		// confronta s con il suo Next rispetto al criterio di ottimizzazione
		// MINTRAVELTIME
	...
	private int compareNIRMinChanges(Stop s, Stop nir) {
		// confronta s con il suo NextInRun rispetto al criterio di
		// ottimizzazione MINCHANGES
	...
	private int compareNISMinChanges(Stop s, Stop nis) {
		// confronta s con il suo NextInStation rispetto al criterio di
		// ottimizzazione MINCHANGES
		// ritorna 1 se s rappresenta un prevSP MIGLIORE per il suo
		// NextInStation
		// ritorna -1 se s rappresenta un prevSP PEGGIORE per il suo
		// NextInStation
	...
	private boolean compareNIR(Stop s, Stop nir) {
		...
		// di default l'ordine dei criteri di ottimizzazione e':
		// - EARLIESTARRIVAL
		// - MINCHANGES
		// - DURATION
		// - MINWALK
		...
		// Scelta minTravelTime (in caso di indecisione rispetto ai criteri
		// precedenti)
		// utile per evitare paths che passano piu volte per la stessa stazione
		if (result == 0)
	...
	private boolean compareNIS(Stop s, Stop nis) {
		...
		// ritorna TRUE se s rappresenta un prevSP migliore per il suo
		// NextInStation
	...
	public class StopExplorer implements StopEvaluator {
		...
		public boolean isStopNode(TraversalPosition tp) {
			...
			if (currentStop.nextInRun != null
					|| currentStop.nextInStation != null) {
				return true;
