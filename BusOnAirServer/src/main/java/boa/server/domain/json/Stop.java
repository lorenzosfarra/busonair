...
public class Stop {
	...
	public Stop(Integer id, Integer staticTime, Integer nextInRun,
			Integer prevInRun, Integer station, Integer run) {
		super();
		this.id = id;
	...
	public Stop(boa.server.domain.Stop s) {
		this(s.getId(), s.getStaticTime(), s.getNextInRun() != null ? s
				.getNextInRun().getId() : null, s.getPrevInRun() != null ? s
				.getPrevInRun().getId() : null, s.getStation().getId(), s
				.getRun().getId());
	}
