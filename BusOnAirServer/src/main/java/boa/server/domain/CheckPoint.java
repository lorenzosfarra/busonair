...
public class CheckPoint {
	...
	public void setLatitude(double lat) {
		// after changing lat/lon call run.updatePosition() to update
		// cpSpatialIndex
	...
	public void setNextCheckPoint(CheckPoint next) {
		...
		if (next != null)
			underlyingNode.createRelationshipTo(next.getUnderlyingNode(),
					RelTypes.NEXTCHECKPOINT);
	}

	public Stop getFrom() {
		Relationship rel = underlyingNode.getSingleRelationship(
				RelTypes.CHECKPOINTFROM, Direction.OUTGOING);
		if (rel == null)
			return null;
		else
	...
	public void setFrom(Stop from) {
		...
		if (from != null)
			underlyingNode.createRelationshipTo(from.getUnderlyingNode(),
					RelTypes.CHECKPOINTFROM);
	}

	public Stop getTowards() {
		Relationship rel = underlyingNode.getSingleRelationship(
				RelTypes.CHECKPOINTTOWARDS, Direction.OUTGOING);
		if (rel == null)
			return null;
		else
			return new Stop(rel.getEndNode());
	...
	public void setTowards(Stop towards) {
		...
		if (towards != null)
			underlyingNode.createRelationshipTo(towards.getUnderlyingNode(),
					RelTypes.CHECKPOINTTOWARDS);
	}
	...
	public String toString() {
		return ("(CPID" + getId() + ":DT" + getDt() + ":TIME"
				+ getTimeInMinutes() + ":FROM STOPID" + getFrom().getId()
				+ ":TOWARDS STOPID" + getTowards().getId() + ")");
	}

	public CheckPoint getPrevCheckPoint() {
		Relationship rel = underlyingNode.getSingleRelationship(
				RelTypes.NEXTCHECKPOINT, Direction.INCOMING);
		if (rel == null)
			return null;
		else
	...
	public String getUrl() {
		return "/runs/" + getFrom().getRun().getId() + "/checkpoints/"
				+ getId();
	}
}
