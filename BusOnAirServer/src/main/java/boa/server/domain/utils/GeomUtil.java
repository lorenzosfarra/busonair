...
public class GeomUtil {
	...
	public static Coordinate proiezione(Coordinate cp1, Coordinate cp2,
			Coordinate cp3) {
		// proietta cp3 sulla retta passante per cp1-cp2
		// cp1 e' il pto in comune tra i 2 segmenti

		double distance = GeoUtil.getDistance2(cp1.y, cp1.x, cp3.y, cp3.x);
		double rapporto = distance
				/ GeoUtil.getDistance2(cp1.y, cp1.x, cp2.y, cp2.x);
