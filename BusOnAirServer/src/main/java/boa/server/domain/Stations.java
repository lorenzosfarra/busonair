...
public class Stations {
	...
	protected Stations() {
		...
		stationSpatialIndex = DbConnection.getSpatialDb()
				.getOrCreatePointLayer("stationSpatialIndex", "lon", "lat");
	}
	...
	public void updateSpatialIndex(Station s) {
		stationSpatialIndex.update(
				...
				stationSpatialIndex.getGeometryFactory().createPoint(
						new Coordinate(s.getLongitude(), s.getLatitude())));
	}
	...
	public void deleteStation(Station s) {
		...
		for (Relationship rel : s.getUnderlyingNode().getRelationships(
				RelTypes.ROUTEFROM, Direction.INCOMING)) {
			Route route = new Route(rel.getStartNode());
			Routes.getRoutes().deleteRoute(route);
		}

		for (Relationship rel : s.getUnderlyingNode().getRelationships(
				RelTypes.ROUTETOWARDS, Direction.INCOMING)) {
			Route route = new Route(rel.getStartNode());
			Routes.getRoutes().deleteRoute(route);
		}
	...
	public Collection<Station> getNearestStations(double lat1, double lon1,
		...
		List<SpatialDatabaseRecord> results = GeoPipeline
				.startNearestNeighborLatLonSearch(stationSpatialIndex,
	...
	public Station createOrUpdateStation(boa.server.domain.json.Station js) {
		...
		if (s != null) { // update (latLon cannot be updated)
			...
			s = new Station(DbConnection.getDb().createNode(), js.getId(),
					js.getName(), js.getLatLon().getLat(), js.getLatLon()
							.getLon());
			addStationToIndex(s);
	...
	public ArrayList<Station> getAllStationsInSpatialIndex() {
		ArrayList<Station> output = new ArrayList<Station>();
		List<SpatialDatabaseRecord> results = GeoPipeline.start(
				stationSpatialIndex).toSpatialDatabaseRecordList();
		for (SpatialDatabaseRecord ris : results) {
	...
	public void linkAllStops(boa.server.domain.json.Stations jsStations) {
		// Link all the stops in the stations
		Stations stations = Stations.getStations();
		for (boa.server.domain.json.Station js : jsStations.stationsObjectsList) {
			Station s = stations.getStationById(js.getId());
			s.linkAllStopsInStation(0, 10080);
		}
	}
}
