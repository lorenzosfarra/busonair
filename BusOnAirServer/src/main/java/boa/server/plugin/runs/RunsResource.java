...
public class RunsResource {
	...
	public Response getAll(@QueryParam("objects") Boolean obj)
		...
		if (obj != null && obj) {
			...
			for (Run r : runs) {
				boa.server.plugin.json.Run jr = new boa.server.plugin.json.Run(
						r);
				runList.add(jr);
			}
	...
	public Response getAllRunningBuses(@QueryParam("objects") Boolean obj)
			throws IOException {
		Iterable<Run> runs = boa.server.domain.Runs.getRuns()
				.getAllRunningBuses();
	...
	public Response getRun(@PathParam("id") Integer id) throws IOException {
		...
		boa.server.domain.Run run = boa.server.domain.Runs.getRuns()
				.getRunById(id);
		...
		if (run != null) {
			...
			return Response
					...
					.entity(new boa.server.plugin.json.Response(404,
							"No run having the specified id value.")).build();
		}
	}
	...
	public Response checkPointPass(@PathParam("id") Integer id,
		...
		if (id == null || checkpointid == null || time == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(400,
							"id, checkpointid, time cannot be blank")).build();
		...
		if (cp == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(404,
							"No checkpoint having the specified id.")).build();
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/{id}/getallstops")
	public Response getAllStops(@PathParam("id") Integer id,
	...
	public Response getAllCheckPoints(@PathParam("id") Integer id,
			@QueryParam("objects") Boolean obj) throws IOException {
	...
	public Response getCheckPointById(@PathParam("id") Integer id,
			@PathParam("idcp") Integer idcp) throws IOException {
		...
		boa.server.plugin.json.CheckPoint jscp = new boa.server.plugin.json.CheckPoint(
				cp);
	...
	public Response getCheckPointTime(@PathParam("id") Integer id,
			@PathParam("idcp") Integer idcp) throws IOException {
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/{id}/rt/restore")
	public Response restore(@PathParam("id") Integer id) throws IOException {
	...
	public Response getLastGPSStop(@PathParam("id") Integer id,
			@QueryParam("delay") Boolean delayParam) throws IOException {
		...
		Boolean delay = (delayParam == null) ? false : delayParam;

		if (!delay) {
			// Don't include the delay in the response
			boa.server.plugin.json.Stop jstop = new boa.server.plugin.json.Stop(
					stop);
			return Response.ok().entity(jstop).build();
		} else {
			boa.server.plugin.json.StopDelay jstopDelay = new boa.server.plugin.json.StopDelay(
					stop, run.getDelay());
			return Response.ok().entity(jstopDelay).build();
			// return Response.ok().entity(jstop).build();
		}
	...
	public Response calculateLastStop(@PathParam("id") Integer id)
			throws IOException {
	...
	public Response getLastGPSCheckPoint(@PathParam("id") Integer id)
		...
		if (cp == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(500,
							"GRAVE: No CheckPoint found.")).build();
	...
	public Response updatePosition(@PathParam("id") Integer id,
			@QueryParam("time") Integer time, @QueryParam("lat") Double lat,
		...
		if (id == null || lat == null || lon == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(400,
							"id, lat, lon cannot be blank")).build();
	...
	public Response addCheckPoint(@PathParam("id") Integer id,
			@QueryParam("time") Integer time, @QueryParam("lat") Double lat,
			@QueryParam("lon") Double lon) throws IOException {
	...
	public Response getLastGPSPosition(@PathParam("id") Integer id)
		...
		if (run != null) {
			boa.server.plugin.json.Position p = new boa.server.plugin.json.Position(
					new boa.server.plugin.json.Coordinate(run
							.getLastGPSLatitude(), run.getLastGPSLongitude()),
					run.getLastUpdateTime());
