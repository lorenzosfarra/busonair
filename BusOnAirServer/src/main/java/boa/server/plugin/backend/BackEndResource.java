...
public class BackEndResource {
	...
	public BackEndResource(@Context Database database,
			...
			@Context Configuration config) throws IOException {
	...
	public BackEndResource(SessionFactoryImpl sessionFactoryImpl,
			Database database, OutputFormat output, Configuration config)
			throws IOException {
	...
	public Response deleteStation(@PathParam("id") Integer id)
		...
		if (staz == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(404,
							"No station having the specified id value."))
					.build();
	...
	public Response deleteRoute(@PathParam("id") Integer id) throws IOException {
		...
		if (route == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(404,
							"No route having the specified id value.")).build();
	...
	public Response deleteCheckPoint(@PathParam("id") Integer id,
	...
	public Response createOrUpdateStation(
			final boa.server.domain.json.Station input) throws IOException {
		Transaction tx = DbConnection.getDb().beginTx();
		try {
	...
	public Response bulkImportStations(
			final boa.server.domain.json.Stations input) throws IOException {
	...
	public Response createOrUpdateStop(final boa.server.domain.json.Stop input)
		...
		if (st < prev.getStaticTime() || st > next.getStaticTime()) {
			boa.server.plugin.json.Response jr = new boa.server.plugin.json.Response(
					422,
					"staticTime must be between prevInRun.staticTime and nextInRun.staticTime");
			return Response.ok().entity(jr).build();
		}
	...
	public Response createOrUpdateCheckPoint(@PathParam("id") Integer id,
			final boa.server.domain.json.CheckPoint input) throws IOException {
	...
	public Response exportAllStations() throws IOException {
		...
		for (Station s : Stations.getStations().getAll()) {
			boa.server.domain.json.Station jsStation = new boa.server.domain.json.Station(
					s);
	...
	public Response exportAllRoutes() throws IOException {
		...
		for (Route r : Routes.getRoutes().getAll()) {
			boa.server.domain.json.Route jsRoute = new boa.server.domain.json.Route(
					r);
	...
	public Response exportAllStops() throws IOException {
		...
		for (Stop s : Stops.getStops().getAll()) {
			boa.server.domain.json.Stop jsStop = new boa.server.domain.json.Stop(
					s);
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/bulkimportall")
	public Response bulkImportAll(final boa.server.domain.json.Boa input)
			throws IOException {
	...
	public Response cleanDb() {
		try {
			...
			return Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity(JsonHelper.createJsonFrom(e1.getMessage())).build();
		}
	}
	...
	@Consumes("application/json")
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/stations/linkallstops")
	public Response linkStations(final boa.server.domain.json.Stations input)
			throws IOException {
		Stations.getStations().linkAllStops(input);
