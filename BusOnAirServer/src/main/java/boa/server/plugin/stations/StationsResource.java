...
public class StationsResource {
	...
	public Response getAll(@QueryParam("objects") Boolean obj)
		...
		ArrayList<Station> stations = boa.server.domain.Stations.getStations()
				.getAll();

		if (obj != null && obj) {
			boa.server.plugin.json.StationsObjects stationList = new boa.server.plugin.json.StationsObjects();
			for (Station s : stations) {
				boa.server.plugin.json.Station js = new boa.server.plugin.json.Station(
						s);
				stationList.add(js);
			}
	...
	public Response getNearestStations(@QueryParam("lat") Double lat,
			@QueryParam("lon") Double lon, @QueryParam("range") Integer range,
		...
		if ((lat == null) || (lon == null))
			return Response
					...
					.entity(new boa.server.plugin.json.Response(400,
							"Lat and Lon cannot be blank")).build();

		Collection<Station> stations;

		if (range == null)
			stations = boa.server.domain.Stations.getStations()
					.getNearestStations(lat, lon);
		else
			stations = boa.server.domain.Stations.getStations()
					.getNearestStations(lat, lon, range);
	...
	public Response getStation(@PathParam("id") Integer id) throws IOException {
		...
		boa.server.domain.Station s = boa.server.domain.Stations.getStations()
				.getStationById(id);
		...
		if (s != null) {
			...
			return Response
					...
					.entity(new boa.server.plugin.json.Response(404,
							"No station having the specified id.")).build();
		}
	}
	...
	public Response getFirstStopFrom(@PathParam("id") Integer id,
	...
	public Response getAllRoutes(@PathParam("id") Integer id,
			@QueryParam("objects") Boolean obj) throws IOException {
