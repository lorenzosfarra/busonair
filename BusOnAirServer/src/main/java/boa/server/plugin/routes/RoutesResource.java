...
public class RoutesResource {
	...
	public Response getAll(@QueryParam("objects") Boolean obj)
		...
		if (obj != null && obj) {
			...
			for (Route r : routes) {
				boa.server.plugin.json.Route jr = new boa.server.plugin.json.Route(
						r);
				routeList.add(jr);
			}
	...
	public Response getRoute(@PathParam("id") Integer id) throws IOException {
		...
		boa.server.domain.Route route = boa.server.domain.Routes.getRoutes()
				.getRouteById(id);
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("{id}/getallruns")
	public Response getAllRuns(@PathParam("id") Integer id,
	...
	public Response getAllStations(@PathParam("id") Integer id,
			@QueryParam("objects") Boolean obj) throws IOException {
		if (id == null)
			return Response
					.ok()
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("{id}/getfirstavailablerun")
	public Response getFirstAvailableRun(@PathParam("id") Integer id)
			throws IOException {
		if (id == null)
			return Response
					.ok()
		...
		if (route == null)
			return Response
					...
					.entity(new boa.server.plugin.json.Response(404,
							"No route having the specified id value.")).build();
		boa.server.domain.Run r = route.getFirstAvailableRun();
		if (r == null)
			return Response
					.ok()
					.entity(new boa.server.plugin.json.Response(404,
							"No run available today for this route.")).build();

		boa.server.plugin.json.FirstAvailableRun jsonRun = new boa.server.plugin.json.FirstAvailableRun(
				r);
		CheckPoint cp = r.getFirstCheckPoint();
		jsonRun.setFirstCheckPointTime(cp.getTimeInSeconds());
		return Response.ok().entity(jsonRun).build();
	}
}
