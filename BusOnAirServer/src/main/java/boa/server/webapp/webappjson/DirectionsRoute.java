...
public class DirectionsRoute {
	...
	public DirectionsRoute(boa.server.domain.Stop start,
			boa.server.domain.Stop end) {
		...
		while (s != null && !s.equals(start)) {
			coords.push(new Coordinate(s.getStation().getLatitude(), s
					.getStation().getLongitude()));
			i++;
			s = s.prevSP;
		}

		coords.push(new Coordinate(s.getStation().getLatitude(), s.getStation()
				.getLongitude()));
	...
	public DirectionsRoute(double deptTime, double cost, List<Node> nodeList) {
		...
		this.deptStation = ((String) nodeList.get(0).getProperty(Stop.STOPNAME))
				.split(",")[0].trim();
		this.arrStation = ((String) nodeList.get(1).getProperty(Stop.STOPNAME))
				.split(",")[0].trim();
		...
		Query query1 = new TermQuery(new Term(Stop.ROUTEID, (String) nodeList
				.get(0).getProperty(Stop.ROUTEID)));
		Query query2 = NumericRangeQuery.newIntRange(Stop.STOPNUM, startNum,
				endNum, true, true);
		BooleanQuery booleanQuery = new BooleanQuery();
		booleanQuery.add(query1, BooleanClause.Occur.MUST);
		booleanQuery.add(query2, BooleanClause.Occur.MUST);
		QueryContext query = new QueryContext(booleanQuery).sort(new Sort(
				new SortField(Stop.STOPNUM, SortField.INT)));

		IndexHits<Node> stops = nodeList.get(0).getGraphDatabase().index()
				.forNodes("stopLayer").query(query);
		this.numOfStops = stops.size();
		this.latLon = new ArrayList<Coordinate>();
		for (Node node : stops)
			latLon.add(new Coordinate((Double) node.getProperty(Stop.LATITUDE),
					(Double) node.getProperty(Stop.LONGITUDE)));
	}
