...
public class DirectionsSearch {
	...
	public DirectionsSearch(SessionFactoryImpl sessionFactoryImpl,
	...
	public Response directions(@QueryParam("lat1") double lat1,
		...
		if (lat1 == 0 || lat1 == 0 || lon1 == 0 || lon2 == 0 || time == 0)
			return Response.serverError().entity("params cannot be blank")
					.build();

		log.write("\ngetDirection?lat1=" + lat1 + "&lon1=" + lon1 + "&lat2="
				+ lat2 + "&lon2=" + lon2 + "&time=" + time);
		log.flush();

		Date start = new Date();

		ShortestPathGeo mysp = new ShortestPathGeo((int) time, 1000, lat1,
				lon1, lat2, lon2, 500, Criteria.MINCHANGES);
		...
		log.write("\ntime to calculate: " + diff + "(" + diffSeconds
				+ " seconds\n\n");
		log.flush();
		...
		boa.server.plugin.json.Direction sp = directs.getDirectionsList()
				.iterator().next();
		List<boa.server.plugin.json.DirectionRoute> routesDirection = sp
				.getRoutes();
		List<boa.server.plugin.json.DirectionWalk> walksDirection = sp
				.getWalks();
		...
		for (boa.server.plugin.json.DirectionRoute rd : routesDirection) {
			boa.server.domain.Stop s1 = cache.get(Stops.getStops().getStopById(
					rd.getDepId()));
			boa.server.domain.Stop s2 = cache.get(Stops.getStops().getStopById(
					rd.getArrId()));
			log.write("\n -> " + s1.getTime());
			log.write("\n -> " + s2.getTime());
			log.flush();
		...
		for (boa.server.plugin.json.DirectionWalk rw : walksDirection) {
			walks.add(new DirectionsWalk(rw.getDeparture().getLat(), rw
					.getDeparture().getLon(), rw.getArrival().getLat(), rw
					.getArrival().getLon(), rw.getDuration()));
		}
