...
public class DBInserter {
	...
	public void addData() {
		...
		hubTransferLayer2SpatialIndex = new LayerNodeIndex(
				"hubTransferLayerSpatialIndex", db,
				new HashMap<String, String>());
		stopLayer = db.index().forNodes("stopLayer");
		stopSpatialIndex = new LayerNodeIndex("stopSpatialIndex", db,
				new HashMap<String, String>());
		addStops();
		addHubs();
		...
		try {
			for (Route route : routes.getRouteList()) {
				...
				while (stopsOnRoute.hasNext()) {
					...
					if (stop.isHub) {
						Hub hub1 = new Hub(db.createNode(), route, stop,
								hubLayer1, !stopsOnRoute.hasNext());
						hubList1.add(hub1);
						Hub hub2 = new Hub(db.createNode(), route, stop,
								hubLayer2, !stopsOnRoute.hasNext());
						hubList2.add(hub2);
	...
	private void connectHubs(List<Hub> hubList1, List<Hub> hubList2) {
		for (int i = 0; i < hubList1.size() - 1; i++) {
			for (int x = i; x < hubList2.size(); x++) {
				Relationship bus = hubList1
						.get(i)
						.getUnderlyingNode()
						.createRelationshipTo(
								hubList2.get(x).getUnderlyingNode(),
								Connection.Type.HUBBUS1);
				bus.setProperty(Connection.COST, hubList2.get(x).getCost()
						- hubList1.get(i).getCost());
			}
		}
	}

	/**
	 * Add connections between hubTransfer1 and hubTransfer2 using
	 * WithinDistanceQuery
	 */
	private void addWalkingConnections() {
		...
		try {
			...
			for (Node hubTransfer1 : hubTransferLayer1.query(Hub.HUBTRANSFERID,
				...
				params.put(LayerNodeIndex.DISTANCE_IN_KM_PARAMETER,
						Connection.FIVEHUNDREDMETRES);
				for (Node spatialRecord : hubTransferLayer2SpatialIndex.query(
						LayerNodeIndex.WITHIN_DISTANCE_QUERY, params)) {
					Node hubTransfer2 = db.getNodeById((Long) spatialRecord
							.getProperty("id"));
					Relationship walk = hubTransfer1.createRelationshipTo(
							hubTransfer2, Connection.Type.WALK2);
					double distanceInKm = (Double) spatialRecord
							.getProperty("distanceInKm");
	 ...
	 */
	private void connectToTransferHubs(Hub hub1, Hub hub2, Route route,
			Stop stop) {
		Transaction tx = db.beginTx();
		try {
			Node hubTransfer1;
			Node hubTransfer2;
			if (hubTransferLayer1.get(Hub.HUBTRANSFERID, hub1.getTransferId())
					.getSingle() == null) {
				hubTransfer1 = db.createNode();
				hubTransfer1.setProperty(Hub.LATITUDE, stop.lat);
				hubTransfer1.setProperty(Hub.LONGITUDE, stop.lon);
				hubTransferLayer1.add(hubTransfer1, Hub.HUBTRANSFERID,
						hub1.getTransferId());
				hubTransfer2 = db.createNode();
				hubTransfer2.setProperty(Hub.LATITUDE, stop.lat);
				hubTransfer2.setProperty(Hub.LONGITUDE, stop.lon);
				hubTransferLayer2.add(hubTransfer2, Hub.HUBTRANSFERID,
						hub1.getTransferId());
				hubTransferLayer2SpatialIndex.add(hubTransfer2, "", "");
			} else {
				hubTransfer1 = hubTransferLayer1.get(Hub.HUBTRANSFERID,
						hub1.getTransferId()).getSingle();
				hubTransfer2 = hubTransferLayer2.get(Hub.HUBTRANSFERID,
						hub1.getTransferId()).getSingle();
			}
		...
		for (Route route : routes.getRouteList()) {
			...
			try {
				for (Stop stop : route.getStopList()) {
					Stop stop1 = new Stop(db.createNode(), route, stop,
							stopLayer);
