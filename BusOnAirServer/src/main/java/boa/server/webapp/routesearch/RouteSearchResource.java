...
public class RouteSearchResource {
	...
	@Path("/")
	public Response routeSearch(@QueryParam("routeId") Integer routeId)
			throws IOException {
		log.write("\nROUTESEARCH(" + routeId + ")");
		log.flush();

		if (routeId == null)
			return Response.serverError().entity("routeId cannot be blank")
					.build();

		boa.server.domain.Route route = boa.server.domain.Routes.getRoutes()
				.getRouteById(routeId);
		if (route == null) {
			return Response.status(400).entity("No Route Found: " + routeId)
					.build();
		}
